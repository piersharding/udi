...
require './common.php';
require HOOKSDIR.'udi/udi_functions.php';
...
$request['page']->drawSubTitle(sprintf('%s: <b>%s</b> %s: <b>%s %s %s (%s)</b>',
	...
	_('File'),$request['udi']->getSource('name'),number_format($request['udi']->getSource('size')),_('bytes'),$type['description']));
...
while (! $request['udi']->eof()) {
	while ($request['template'] = $request['udi']->readEntry()) {
	...
	if ($request['udi']->error) {
		printf('<small><span style="color:red;">%s: %s</span></small><br />',
			_('Error'),$request['udi']->error['message']);

		echo '<br/>';

		display_pla_parse_error($request['udi']);
	}
